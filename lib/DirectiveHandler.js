...
module.exports = (function() {
	...
	DirectiveHandler.prototype = {
		...
		handleDirective: function(directive, directiveName, currentFile, variables, includeStack) {
			...
			switch (directiveName) {
				...
				case "include":
					interpolate.apply(this);
					return this._handleInclude(attributes, currentFile, variables, includeStack);
				default:
					interpolate.apply(this);
					return this._handleCustom(directiveName, attributes, currentFile, variables);
			}
		...
		/* handle Custom addon */
		_handleCustom: function(directiveName, attributes, currentFile, variables) {
			var customHandler = this.parser.customHandlers[directiveName] || DirectiveHandler.handlers[directiveName]
			if(customHandler) {
				var content = customHandler(attributes, currentFile, variables)
				if(!content) {
					return {error: "Could not find parse directive #" + directiveName};
				} else {
		...
		_handleInclude: function(attributes, currentFile, variables, includeStack) {
			...
			var parsed = this.parser.parse(this.ioUtils.resolveFullPath(currentFile, filename), results.output, variables, includeStack);
